    ...
    public sealed class JobExtensionL0
    {
        ...
        public async Task JobExtensioIntraTaskState()
        {
            using (TestHostContext hc = CreateTestContext())
            {
                ...
                result.PreJobSteps[0].ExecutionContext.SetIntraTaskStates("state1", "value1", false);
                Assert.Equal("value1", result.JobSteps[1].ExecutionContext.GetIntraTaskStates().Get("state1"));
                Assert.Equal("value1", result.PostJobStep[3].ExecutionContext.GetIntraTaskStates().Get("state1"));

                Assert.Null(result.JobSteps[0].ExecutionContext.GetIntraTaskStates().Get("state1"));
                Assert.Null(result.PreJobSteps[1].ExecutionContext.GetIntraTaskStates().Get("state1"));
                Assert.Null(result.PreJobSteps[2].ExecutionContext.GetIntraTaskStates().Get("state1"));
                Assert.Null(result.PostJobStep[2].ExecutionContext.GetIntraTaskStates().Get("state1"));
                Assert.Null(result.JobSteps[2].ExecutionContext.GetIntraTaskStates().Get("state1"));
                Assert.Null(result.JobSteps[3].ExecutionContext.GetIntraTaskStates().Get("state1"));
            }
        }
    }
}
